...
export default class restClient {
    ...
    */
    ...
    /**
     * gets a project version by name
     * @param {*} parentId
     * @param {*} projectVersionName
     */
    getProjectVersion(parentId, projectVersionName) {
            const restClient = this;
            return new Promise((resolve, reject) => {
                if (!restClient.api) {
                    return reject("restClient not initialized! make sure to call initialize before using API");
                }
                restClient.api["project-version-of-project-controller"].listProjectVersionOfProject({
                    parentId: parentId, 
                    q: projectVersionName,
                    fulltextsearch: true
                }, getClientAuthTokenObj(restClient.token)).then((resp) => {
                    resolve(resp.obj.data);
                }).catch((error) => {
